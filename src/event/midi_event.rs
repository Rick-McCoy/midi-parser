    ...
    pub fn parse(input: &[u8], status: u8) -> IResult<&[u8], Self> {
        ...
        match message_type {
            ...
            0xa => {
                ...
                assert_eq!(note >> 7, 0, "Invalid note: {:x}", note);
                assert_eq!(pressure >> 7, 0, "Invalid pressure: {:x}", pressure);
                Ok((
                    input,
            ...
            0xb => {
                ...
                assert_eq!(controller >> 7, 0, "Invalid controller: {:x}", controller);
                assert_eq!(value >> 7, 0, "Invalid value: {:x}", value);
                Ok((
                    input,
                    Self::ControlChange {
                        channel,
            ...
            0xc => {
                ...
                assert_eq!(program >> 7, 0, "Invalid program: {:x}", program);
            ...
            0xe => {
                ...
                assert_eq!(lsb >> 7, 0, "Invalid lsb: {:x}", lsb);
                assert_eq!(msb >> 7, 0, "Invalid msb: {:x}", msb);
...
impl ChannelMessage {
    pub fn parse(input: &[u8], status: u8) -> IResult<&[u8], Self> {
        ...
        match message_type {
            ...
            0xb => {
                let (input, controller) = peek(be_u8)(input)?;
                match controller {
    ...
    pub fn parse(input: &[u8], status: u8) -> IResult<&[u8], Self> {
        ...
        match message_type {
            0x2 => {
                let (input, lsb) = be_u8(input)?;
                let (input, msb) = be_u8(input)?;
                assert_eq!(lsb >> 7, 0, "Invalid lsb: {}", lsb);
                assert_eq!(msb >> 7, 0, "Invalid msb: {}", msb);
                let value = ((msb as u16) << 7) | (lsb as u16);
                Ok((input, Self::SongPositionPointer { value }))
            }
            0x3 => {
                let (input, song) = be_u8(input)?;
                assert_eq!(song >> 7, 0, "Invalid song: {}", song);
